...
!macro preInit
    SetRegView 64
    WriteRegExpandStr HKLM "${INSTALL_REGISTRY_KEY}" InstallLocation "$LOCALAPPDATA\Covision\eumtalk\"
    WriteRegExpandStr HKCU "${INSTALL_REGISTRY_KEY}" InstallLocation "$LOCALAPPDATA\Covision\eumtalk\"
    SetRegView 32
    WriteRegExpandStr HKLM "${INSTALL_REGISTRY_KEY}" InstallLocation "$LOCALAPPDATA\Covision\eumtalk\"
    WriteRegExpandStr HKCU "${INSTALL_REGISTRY_KEY}" InstallLocation "$LOCALAPPDATA\Covision\eumtalk\"
!macroend
...
!macro customInstall
  ...
  ${IfNot} ${isUpdated}
    ${If} ${RunningX64}
        ...
        MessageBox MB_YESNO "eumtalk 사용을 위하여\
        'Microsoft Visual C++ Redistributable for Visual Studio 2015 - 2019 x64'\
        이(가) 요구됩니다.$\r$\n$\r$\n\
        지금 설치하시겠습니까?$\r$\n(이미 설치된경우 설치되지 않습니다.)" /SD IDYES IDNO VSRedistInstalled

        StrCpy $VCRedistFileName "vc_redist.x64.exe"
    ${Else}
        ...
        StrCpy $VCRedistFileName "vc_redist.x86.exe"
    ${EndIf}
  
  ;if no goto executed, install vcredist
  ;exec installer
  ExecWait "$INSTDIR\resources\vcredist\$VCRedistFileName"
