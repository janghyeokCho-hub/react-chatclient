...
const notifier = path.resolve('node_modules/node-notifier/vendor/snoreToast/snoretoast');
...
function notifyRaw(options, callback) {
  ...
  console.log('what option???', options)
  console.log('what server???', server)
  ...
  const actionJackedCallback = (err) =>{
    ...
    return snoreToastResultParser(
      err,
      utils.actionJackerDecorator(this, options, callback, (data) =>{
        console.log('DATA???????????????????????', data)
        return data === 'activate' ? 'click' : data || false;
      })
    )};
