...
import Emoji from '@C/chat/chatroom/controls/emoji/data/emoji';
import Scrollbars from 'react-custom-scrollbars';

const EmojiLayer = ({ onAppend }) => {
  ...
  const handleAppend = emoji => {
    let newRecentEmoji = recentEmoji.filter(item => {
      return item.e != emoji.e;
    });

    newRecentEmoji = [emoji, ...newRecentEmoji].splice(0, 10);

    setRecentEmoji(newRecentEmoji);
    localStorage.setItem('recent-use-emoji', JSON.stringify(newRecentEmoji));
    onAppend(emoji.e);
  };

  useEffect(() => {
    const saveRecentEmoji = localStorage.getItem('recent-use-emoji');
    if (saveRecentEmoji) {
      try {
        const parsingData = JSON.parse(saveRecentEmoji);
        setRecentEmoji(parsingData);
      } catch (e) {
        // console.log('load recent emoji error');
      }
    }
  }, []);
      ...
      <div className="chat_sticker">
        <div className="chat_sticker_tab">
          <ul className="chat_lst_sticker_set">
            <li
              className={(category == 'recent' && 'on') || ''}
              onClick={e => {
                handleCategory('recent');
              }}
            >
              <button tyle="button" className="stickerPackBtn">
                <svg
                  aria-hidden="true"
                  focusable="false"
                  data-prefix="fas"
            ...
            <li
              className={(category == 'smileys' && 'on') || ''}
              onClick={e => {
                handleCategory('smileys');
              }}
            >
              <button tyle="button" className="stickerPackBtn">
                <svg
            ...
            <li
              className={(category == 'animals' && 'on') || ''}
              onClick={e => {
                handleCategory('animals');
              }}
            >
              <button tyle="button" className="stickerPackBtn">
                <svg
            ...
            <li
              className={(category == 'activities' && 'on') || ''}
              onClick={e => {
            ...
            <li
              className={(category == 'travel' && 'on') || ''}
              onClick={e => {
                handleCategory('travel');
              }}
            >
              <button tyle="button" className="stickerPackBtn">
                <svg
            ...
            <li
              className={(category == 'objects' && 'on') || ''}
              onClick={e => {
                handleCategory('objects');
              }}
            >
              <button tyle="button" className="stickerPackBtn">
                <svg
            ...
            <li
              className={(category == 'symbols' && 'on') || ''}
              onClick={e => {
                handleCategory('symbols');
              }}
            >
              <button tyle="button" className="stickerPackBtn">
                <svg
