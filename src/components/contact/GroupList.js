...
const GroupList = ({ checkObj, group }) => {
    ...
    e => {
      ...
      if (text !== '') {
        debounceTimer.takeLatest(() => {
          ...
          */
          setSearchResult(
            group.sub.filter(user => user.name.split(';')[0].includes(text)),
          );
        }, 200);
  ...
  return (
    <>
      <SearchBar
        placeholder={
          getDictionary(group.folderName) +
          ' ' +
          covi.getDic('Search_Member', '멤버 검색')
        }
        input={searchText}
      ...
      <Scrollbars
        ...
        style={{
          height:
            'calc(100% - ' +
            (checkObj?.checkedList?.length > 0 ? '220px' : '124px') +
            ')',
          display: 'none',
        }}
      >
