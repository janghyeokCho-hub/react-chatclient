...
export default function useCopyWithSenderInfo() {
  ...
  const copyWithSenderInfo = messages => {
    ...
    let firstSelectedMessage = range.startContainer.parentNode.parentNode.getAttribute(
      ATTRIBUTE_NAME,
    );
    let tempFindNode = range.endContainer;
    let lastSelectedMessage = range.endContainer.parentNode.parentNode.getAttribute(
      ATTRIBUTE_NAME,
    );
    if (firstSelectedMessage === null)
      firstSelectedMessage = range.startContainer.parentNode.parentNode.parentNode.getAttribute(
        ATTRIBUTE_NAME,
      );
    if (lastSelectedMessage === null)
      lastSelectedMessage = range.endContainer.parentNode.parentNode.parentNode.getAttribute(
        ATTRIBUTE_NAME,
      );
    if (lastSelectedMessage === null) {
      if (lastSelectedMessage == null) {
        if (
          tempFindNode.parentNode.parentNode.parentNode.previousSibling !== null
        ) {
          lastSelectedMessage = tempFindNode.parentNode.parentNode.parentNode.previousSibling.childNodes[1].childNodes[0].getAttribute(
            ATTRIBUTE_NAME,
          );
        }
      }
    ...
    allMessageID.forEach(element => {
      ...
      if (
        firstSelectedMessage <= selectedMessage &&
        selectedMessage <= lastSelectedMessage
      ) {
      ...
      arr.forEach(element => {
        ...
        if (mid === ele.messageID) {
          ...
          msg =
            ...
            ']' +
            format(new Date(ele.sendDate), '[yyyy/MM/dd - HH:mm:ss]') +
            ' ' +
            ele.context;
          msgArr.push(msg);
        }
