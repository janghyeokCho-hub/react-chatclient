...
export function useViewState(initialData = { type: null, noteId: null, noteInfo: null }) {
...
export function useNoteState(noteId) {
    ...
    const state = useSWR(KEY, noteId ? () => getNote(noteId) : null, { revalidateOnFocus: false });
...
export function useNoteUnreadCount() {
    ...
    const { data:unreadCnt, mutate:setUnreadCnt } = useSWR('/note/list/receive/unread', null);
...
// 파일 다운로드
export function downloadFile({ userId = null, accessKey = null, serviceType = 'NOTE', downloadHandler = () => {}}) {
    const module = 'CR';
...
export function sendNote({ sender, receiveUser = [], receiveGroup = [], subject, context, files = [], fileInfos = [], isEmergency }) {
    ...
    formData.append('receiveGroup', AESUtil.encrypt(JSON.stringify(receiveGroup)));
