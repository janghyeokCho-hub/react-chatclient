...
export default function ReadListTable({ noteId, onError }) {
  ...
  const tableData = useMemo(() => {
    ...
    const columns = [
      ...
      {
        title: covi.getDic('Read', '읽음'),
        field: 'readFlag',
        render: rowData => {
          const txtColor = rowData.readFlag === 'Y' ? '#1976d2' : '#F86A60';
          const txt =
            rowData.readFlag === 'Y'
              ? covi.getDic('Check', '확인')
              : covi.getDic('Uncheck', '미확인');
    ...
    const data =
      ...
      readList.map(status => {
        return {
          ...
          name: `${status.displayName || 'UNKNOWN'} ${
            status.jobLevelName || ''
          }`,
  ...
  if (readList === null) {
    ...
    return (
      <Scrollbars autoHide={true}>
        <MaterialTable
          title={`Total: ${tableData.total} | ${covi.getDic(
            'Check',
            '확인',
          )}: ${tableData.read} | ${covi.getDic('Uncheck', '미확인')}: ${
            tableData.unread
          }`}
