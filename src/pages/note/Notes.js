...
import { useNoteState, useViewState, useNoteList, deleteNote, archiveNote, setFavorite, translateName, parseSender, convertTimeFormat, emergencyMark } from '@/lib/note';
...
function _NoteItem({ note, viewType, history }) {
    ...
    // 쪽지 발신창 열기
    async function _openNewNote(dispatch, type, noteInfo, { viewer = false, newWin = false } = {}) {
    ...
    const menus = useMemo(() => {
        ...
        const favoriteMenu = {
            ...
            sop: isFavorite ? 'D' : 'C',
            successMessage: isFavorite ? covi.getDic('Msg_Note_FavoriteDeleteSuccess', '즐겨찾기에서 삭제했습니다.') : covi.getDic('Msg_Note_FavoriteCreateSuccess', '즐겨찾기에 추가되었습니다.'),
            name: isFavorite ? covi.getDic('Msg_Note_FavoriteDelete', '즐겨찾기 삭제') : covi.getDic('Msg_Note_FavoriteCreate', '즐겨찾기')
        };
        ...
        const _menus = [
            {
                ...
                onClick() {
                    ...
                    _openNewNote(dispatch, 'receive', note, { newWin: deviceViewType === 'S' });
                },
            ...
            },
            {
                ...
                async onClick() {
                    // todo
                    try {
                        const { data } = await setFavorite({ noteId: note.noteId, sop: favoriteMenu.sop });
                        ...
                        popupResult(dispatch, covi.getDic('Msg_Note_FavoriteFail', '즐겨찾기 처리에 실패했습니다. 다시 시도해주세요'));
                    }
                },
                ...
                async onClick() {
                    ...
                    try {
                        ...
                        if (data && data.status === 'SUCCESS') {
                            ...
                            popupResult(dispatch, covi.getDic('Msg_Note_DeleteSuccess', '쪽지가 삭제되었습니다.'));
                        } else {
        ...
        if (viewType !== 'archive') {
            _menus.push({
                ...
                async onClick() {
                    // todo
                    try {
                        const { data } = await archiveNote({ noteId: note.noteId, sop: 'C' });
                        if (data && data.status === 'SUCCESS') {
                            popupResult(dispatch, covi.getDic('Msg_Note_ArchiveCreateSuccess', '쪽지가 보관되었습니다.'));
                        } else {
        ...
        <RightContextMenu menuId={"Note_" + note.noteId} menus={menus}>
            ...
            >
                <a>
                    ...
                    <span className={clsx("tit", note.readFlag === 'Y' ? 'read' : 'unread')} style={{ maxWidth: 'calc(100% - 200px)' }}>
                        {note?.emergency === 'Y' && emergencyMark}
                        {note.subject}
                    </span>
                    {note.readFlag === 'N' && (<span style={{ marginLeft: '3px', color: '#F86A60', fontWeight: 'bold', fontSize: '11px' }} >N</span>)}
                    {/* <span className="team">{nameInfo}</span> */}
                    <span className="team">{translateName(senderInfo)}</span>
                    <span className="date" style={{ maxWidth: '135px' }}>{noteTime}</span>
                </a>
                <div className="fuction_btn">
                    <a className={clsx("btn_file", note.fileFlag === 'Y' && 'active')} style={{ marginRight: '3.5px' }}></a>
                    <a className={clsx("btn_note", note.readFlag === 'N' && 'active')} style={{ marginRight: '3.5px' }}></a>
                    <a className={clsx("btn_favorite", note.favorites === '1' && 'active')}></a>
                </div>
            </li>
        </RightContextMenu>
    );
...
export default function Notes({ viewType, noteList }) {
    const initialNumToRender = window.innerWidth ? Math.ceil(window.innerWidth / 60) + 2 : 15;
    const renderPerBatch = 5;
    const { handleScrollUpdate, list } = useOffset(noteList, { initialNumToRender, renderPerBatch });
    const handleUpdate = handleScrollUpdate({ threshold: 0.85 });

    const drawList = list && list((note, idx) => <NoteItem key={idx} note={note} viewType={viewType} />);

    return (
        <Scrollbars autoHide={true} onUpdate={handleUpdate} className="MessageList" style={{ height: 'calc(100% - 210px)' }}>
            <ul className="people">
                {drawList}
            </ul>
        </Scrollbars>
