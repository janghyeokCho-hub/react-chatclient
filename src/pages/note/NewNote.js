import React, { useState, useEffect, useLayoutEffect, createRef, useCallback, useMemo } from 'react';
...
export default function NewNote({ match, location }) {
    ...
    const params = isNewWin && location.search && qs.parse(location.search, { ignoreQueryPrefix: true });
    ...
    const setReplyTarget = useCallback(async (userInfo, { replyAll = false }) => {
        const nextTargets = [];
    ...
    const replyContext = useMemo(() => {
        ...
        const replyHTML = noteInfo.context.split('<br>').map(txt => `<p>${txt}</p>`).join('');
        return replyFormat + replyHTML;
    }, [viewState]);
    ...
    useLayoutEffect(() => {
        ...
        if(isNewWin && replyContext) {
            editorRef.current.getInstance().setHTML(replyContext);
        ...
        if (noteInfo) {
            ...
            const userInfo = parseSender(noteInfo, { useOrgChartFormat: true, removePresence: true });
    ...
    const addTarget = useCallback(() => {
        appendLayer(
            {
                component: <AddTarget oldMemberList={targets} onChange={(changedTargets) => setTargets(changedTargets)} />
            },
            dispatch,
        );
    }, [viewState, targets]);
    ...
    function handleFileChange(e) {
        ...
        if (target && target.files.length > 0) {
            ...
            if (appendResult.result == 'SUCCESS') {
                ...
                _popupResult(dispatch, coviFile.getValidationMessage(appendResult.message));
            }
        }
    }
    ...
    async function handleSend() {
        ...
        targets.forEach(target => {
            if (target.type === 'U') {
                // 유저: '{id}|{kobKey}'
                receiveUser.push(`${target.id}${NOTE_RECEIVER_SEPARATOR}${target.jobKey}`);
            } else if (target.type === 'G') {
                // 그룹: '{id}|{companyCode}'
                receiveGroup.push(`${target.id}${NOTE_RECEIVER_SEPARATOR}${target.companyCode}`);
            }
        });
        ...
        if (receiveUser.length === 0 && receiveGroup.length === 0) {
            _popupResult(dispatch, covi.getDic('Msg_Note_EnterRecipient', '받는사람을 선택하세요.'));
            return;
        }
        if (title.current.value.trim().length === 0) {
            _popupResult(dispatch, covi.getDic('Msg_Note_EnterTitle', '제목을 입력하세요.'));
            return;
        } else if (title.current.value.length > 1900) {
            _popupResult(dispatch, covi.getDic('Msg_Note_TitleExceeded', '제목은 1900자 이하로 입력하세요.'));
            return;
        }
        // if (context.current.value.trim().length === 0) {
        if (_context.trim().length === 0) {
            _popupResult(dispatch, covi.getDic('Msg_Note)EnterContext', '내용을 입력하세요.'));
            return;
        }
        //
        ...
        try {
            ...
            if (data && typeof data.result !== 'undefined') {
                ...
                _popupResult(dispatch, covi.getDic('Msg_Note_SendSuccess', '쪽지 전송에 성공했습니다.'), () => {
                    if (DEVICE_TYPE === 'b') {
                        // 쪽지발신 뷰 닫기
                        clearViewState();
                    } else if (DEVICE_TYPE === 'd') {
                ...
                _popupResult(dispatch, covi.getDic('Msg_Note_SendFail', '쪽지 전송에 실패했습니다.'));
            }
        } catch (err) {
            ...
            <div id="wrap" style={{ height: 'calc(100% - 60px)' }}>
                <Scrollbars autoHide={true} className="noteWrap Layer-Notepop" style={{ zIndex: 'unset', overflow: 'hidden' }}>
                        ...
                        <ul>
                            {
                                targets.map((target, idx) => {
                                    return (
                                        <li key={idx}>
                                            <a className="ui-link" onClick={() => removeTarget(target.name)}>
                                                <ProfileBox
                                                    userId={target.id}
                    ...
                    <div className="Layer-Note-Con" style={{ marginBottom: '60px' }}>
                        <div className="Profile-info-input" style={{ textAlign: 'start' }}>
                            ...
                            <div className="input full">
                                <label className="string optional" htmlFor="user-name" style={{ cursor: 'inherit' }}>내용</label>
                            ...
                            <div className="input full">
                                <label className="string optional file" htmlFor="user-name" onClick={(e) => {
                                    fileUploadControl.current.click();
                                    e.stopPropagation();
                                }}>{covi.getDic("AttachFile")}</label>
                                <ul className="file-list_note type01" style={{ overflowY: 'scroll' }}>
                ...
                <div className="layer-bottom-btn-wrap right" style={{ zIndex: 499 }}>
                    <a className="Btn-pointcolor-mini" onClick={handleSend} >
                        {isSending ? <TailSpin style={{ verticalAlign: 'middle' }} /> : covi.getDic('Msg_Note_Send', '보내기')}
                    </a>
                </div>
            </div>
        </ConditionalWrapper>
