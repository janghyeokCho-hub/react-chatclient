...
export default function AddTarget({ oldMemberList, onChange, onRemove }) {
    ...
    return (
        <div key='AddTarget' className="Layer-AddUser" style={{ height: '100%', minWidth: '400px' }}>
            <div className="modalheader">
                ...
                <a className="Okbtn" onClick={handleChange}>
                    <span className="colortxt-point mr5">{members.length + oldMembers.length}</span>
                    {covi.getDic('Ok')}
                </a>
            </div>
            <div className="container AddUser">
                    ...
                    <ul>
                        {
                            members.map((member, idx) => {
                                return (
                                    <li key={idx}>
                                        <a className="ui-link" onClick={() => delUser('jobKey', member.jobKey)}>
                                            <ProfileBox
                                                userId={member.id}
                ...
                <div className={['tabcontent', selectTab == 'C' ? 'active' : ''].join(' ')}>
                    <div className="AddUserCon">
                        <ContactList viewType="checklist" checkObj={checkObj} />
                    </div>
                </div>
                <div className={['tabcontent', selectTab == 'O' ? 'active' : ''].join(' ')}>
                    <div className="AddUserCon">
