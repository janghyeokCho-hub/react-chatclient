  ...
  const menus = [
    {
      ...
      name: (
        <SortButton
          text={covi.getDic('Date', '날짜')}
          active={activeSort === SORT.DATE}
          direction={dateSort}
        />
      ),
      preventClose: true,
    ...
    },
    {
      ...
      name: (
        <SortButton
          text={covi.getDic('Note_Sender', '보낸사람')}
          active={activeSort === SORT.NAME}
          direction={nameSort}
        />
      ),
      preventClose: true,
  ...
  return (
    <div className="note_info">
      <p className="txt" style={{ display: 'inline-block' }}>
        <strong className="count">({unreadNoteCnt})</strong>
        {collapseInfo
          ? covi.getDic('Msg_Note_Unread_Short', '읽지 않음')
          : covi.getDic('Msg_Note_Unread', '개의 읽지 않은 쪽지가 있습니다.')}
      </p>
      <div
        className="note_btn"
        style={{ display: 'inline-block', top: 'unset', transform: 'unset' }}
      >
        <RightContextMenu
          menuId={menuId}
          menus={menus}
          holdToDisplay={0}
          renderTag="a"
          attributes={{ className: 'btn_sort' }}
        >
          {covi.getDic('Sort', '정렬')}
        </RightContextMenu>
        &nbsp;
        <a className="btn_delete" onClick={deleteAllNotes}>
          {covi.getDic('DeleteAll', '전체삭제')}
        </a>
      </div>
    </div>
  );
